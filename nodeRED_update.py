...
#ITERATE OVER THE CSV LIST (# of iterations = # of rows)
  for row in csvreader:
    ...
    try:
        ...
        time.sleep(1)
        print("INF: Updating NodeRED on server: {}".format(url)+" ...")
        time.sleep(2)
        print("INF: NodeRED on {}".format(url)+" has been sucessfully updated!")
        time.sleep(4)
        print(response.status_code)
    except requests.exceptions.HTTPError as err:
        print("WAR: NodeRED on server {} has already been updated".format(url))
        time.sleep(4)
